...
csv_file = sys.argv[1] if len(sys.argv)>1 else '/Users/landerson2/Desktop/static_color.csv'
...
with open(csv_file) as csv:
	for _ in csv.readlines():
		# PRODUCT_ID, FILE_NAME, SUFFIX, COLORIZATION_FILE_NAME, SKU_ID, OPTION_ID, SALE_SWATCH, RH_FULL_SKU_ID, DISPLAY_NAME, PRODUCT_HEIGHT, PRODUCT_LENGTH, PRODUCT_WIDTH
		prod_id, filename, suffix, colorization_file_name, sku_id, option_id, swatch_id, rh_full_sku_id, display_name, product_height, product_length, product_width = _.replace('"',"").split(",")
		...
		for i in range(1): # SWITCH TO TWO (2) FOR LEGACY AND RHR
			...
			if original_file == None: 
				continue
			new_name = f"{colorization_file_name}{_rhr}_cl{swatch_id}_{option_id}"
			# print(new_name)
			...
			# crop the originating file
			# new_file = trim(original_file) # PIL IMAGE
